...
void obstacleAvoidanceUpdate() 
{
 ...
 pChoquesDir = farrusco.PChoqueDirValor();
  ...
  // 1 = pará choques esquerdo // 2 = pará choques direito // 0 = Nenhum dos pará choques foi pressionado
  if (!checkDeadEnd())  // Igual a (checkDeadEnd() == 0)
  {  
    velocidadeMotorDir = maxVelocidadeDir;
    velocidadeMotorEsq = maxVelocidadeEsq;
    
    //aumenta a velocidade do servo e aumenta o valor máximo de rotação
    servoDel = 45;
    servoMin = 20;
    servoMax = 160;
    ...
    {   
      if (servoPos < 90) 
      {
        ...
        //diminui a velocidade dos servo e altera o valores máx e min de rotação
        servoDel = 250;
        servoMin = 20;
        servoMax = 80;
      }
